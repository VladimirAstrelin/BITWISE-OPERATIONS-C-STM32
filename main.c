...
/**
  ******************************************************************************
  * @file           : main.c
  * @brief          : Bitwise operations demo
  ******************************************************************************
  * @attention		: 30-08-2025
  * @author			: Vlad Astrelin
  ******************************************************************************
  */
...
// Функция для отображения битового (бинарного) представления числа с дефисом
void Display_Binary_Value(uint8_t value) {
    char binary_str[10]; // 8 бит + дефис + нулевой символ
    char display_str[17]; // Строка для дисплея

    // Сначала формируем старшие 4 бита (биты 7-4)
    for (int i = 7; i >= 4; i--) {
    ...
    // Затем формируем младшие 4 бита (биты 3-0)
    for (int i = 3; i >= 0; i--) {
        binary_str[5 + (3-i)] = (value & (1 << i)) ? '1' : '0';
    }
    ...
    snprintf(display_str, sizeof(display_str), "BIN:%s", binary_str);
  ...
  while (1)
  {
    ...
    if (strcmp(button, "NONE") != 0) {
        ...
        if (strcmp(button, "LEFT") == 0) {
            ...
            current_value <<= 1;
            if (current_value == 0) current_value = 0x01; // Предотвращаем обнуление
        }
        else if (strcmp(button, "RIGHT") == 0) {
            // Сдвиг вправо (деление на 2)
            current_value >>= 1;
            if (current_value == 0) current_value = 0x80; // Предотвращаем обнуление
        }
